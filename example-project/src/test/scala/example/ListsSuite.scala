...
class ListsSuite extends FunSuite {
  ...
  test("sum of a list which contains negative numbers") {
  ...
  test("max throw a NoSuchElementException is the list is empty") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }
  
  test("max of a list with repeated numbers") {
    assert(max(List(1,2,2,3,3)) === 3)
  }
}
